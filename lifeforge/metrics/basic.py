"""Basic macroscopic cellular automata metrics (population, density, step activity)."""
...
def activity(previous: World, current: World) -> float:
    """Return the fraction of cells whose state changed between two consecutive generations."""
    if previous.width != current.width or previous.height != current.height:
        raise ValueError("World dimensions must match.")

    changed = np.count_nonzero(previous.state != current.state)
